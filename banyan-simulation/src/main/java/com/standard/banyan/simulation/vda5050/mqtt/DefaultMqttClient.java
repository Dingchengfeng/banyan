...
import com.standard.banyan.simulation.vda5050.topic.PublishTopic;
import com.standard.banyan.simulation.vda5050.topic.Topic;
...
public class DefaultMqttClient {
    ...
    public void connect(MqttCallback mqttCallback, Topic willTopic,String willMessage) {
        MqttConnectionOptions connOpts = new MqttConnectionOptions();
        connOpts.setUserName(this.username);
        connOpts.setPassword(this.password.getBytes());
        connOpts.setCleanStart(true);
        ...
        mqttMessage.setPayload(willMessage.getBytes(StandardCharsets.UTF_8));
        mqttMessage.setQos(willTopic.getQos().getQosValue());
        mqttMessage.setRetained(willTopic.isRetained());
        connOpts.setWill(willTopic.getTopicFilter(),mqttMessage);
        mqttClient.setCallback(mqttCallback);
        try {
            mqttClient.connect(connOpts);
        } catch (MqttException e) {
